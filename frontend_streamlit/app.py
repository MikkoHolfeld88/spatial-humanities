...
if on_stadtteile_leipzig:
    map_service.add_data_to_map(json.loads(regions), name="regions")

if on_krankenhauser_leipzig:
    map_service.add_data_to_map(json.loads(hospitals), name="hospitals")

map_1 = map_service.get_map()
keplergl_static(map_1)

def activate_scenario(index):
    result = calculation_service.calculate(Store.get_scenario(index), regions, hospitals)
